...
import { 
    ...
    DollarSign, RefreshCcw, Users, Star, Store, UserCheck 
} from 'lucide-react';
...
export const AdminDashboard = () => {
    ...
    if (error) return (
        <div className="p-10 text-center bg-[#0a0a0a] min-h-screen flex flex-col items-center justify-center">
            ...
            <button onClick={loadData} className="px-5 py-2.5 bg-white text-black text-[9px] font-black uppercase tracking-widest rounded-sm hover:bg-red-500 hover:text-white transition-all">
                Reintentar Conexión
            </button>
        </div>
    );

    const maxSearch = Math.max(...(stats.top_searches?.map(s => s.count) || [1]), 1);
    const maxDemand = Math.max(...(stats.stock_demands?.map(d => d.total) || [1]), 1);
    ...
    return (
        <div className="p-6 space-y-6 bg-[#0a0a0a] min-h-screen text-gray-300">
            ...
            <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-gray-900 pb-6">
                <div>
                    <div className="flex items-center gap-2 mb-1 text-purple-500">
                        <Activity size={14} />
                        <span className="text-[9px] font-black uppercase tracking-[0.4em]">Core Analytics v2.0</span>
                    </div>
                    <h1 className="text-3xl font-black text-white tracking-tighter uppercase italic">
                        Control <span className="text-gray-600">Center</span>
                    </h1>
                </div>
                <button onClick={loadData} className="flex items-center gap-2 px-4 py-2 bg-[#111] border border-gray-800 rounded-lg text-[9px] font-black uppercase tracking-widest hover:border-purple-500/50 hover:text-white transition-all group shadow-lg">
                    <RefreshCcw size={12} className="group-active:rotate-180 transition-transform duration-500" />
                    Sync Data
                </button>
            </header>

            {/* FILA 1: KPIs DE USUARIOS E INVENTARIO */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard 
                    label="Clientes Activos" 
                    value={stats.user_summary.clientes} 
                    icon={<UserCheck size={18} />} 
                    color="text-emerald-400" bg="bg-emerald-400/5"
                />
                <StatCard 
                    label="Proveedores" 
                    value={stats.user_summary.proveedores} 
                    icon={<Users size={18} />} 
                    color="text-blue-400" bg="bg-blue-400/5"
                />
                <StatCard 
                    label="Stock Crítico" 
                    value={stats.inventory_summary.out_of_stock_count} 
                    icon={<AlertTriangle size={18} />} 
                    color="text-red-400" bg="bg-red-400/5"
                />
                <StatCard 
                    label="Valoración Total" 
                    value={`$${stats.inventory_summary.total_value.toLocaleString()}`} 
                    icon={<DollarSign size={18} />} 
                    color="text-amber-400" bg="bg-amber-400/5"
                />
            </div>

            {/* FILA 2: RANKING DE TIENDAS (NUEVO) */}
            <section className="bg-[#111] p-6 rounded-lg border border-gray-800">
                <h2 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] flex items-center gap-2 mb-6">
                    <Store size={14} className="text-purple-500" /> Tiendas Mejor Calificadas
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                    {stats.top_stores.length > 0 ? stats.top_stores.map((store, idx) => (
                        <div key={idx} className="bg-[#0c0c0c] p-4 rounded border border-gray-900 hover:border-purple-500/30 transition-all group">
                            <p className="text-[10px] font-bold text-white uppercase truncate mb-1">{store.name}</p>
                            <div className="flex items-center gap-2">
                                <div className="flex items-center text-amber-400">
                                    <Star size={10} className="fill-current" />
                                    <span className="text-xs font-mono ml-1">{store.rating}</span>
                                </div>
                                <span className="text-[9px] text-gray-600 uppercase font-bold">{store.review_count} reviews</span>
                            </div>
                            <div className="mt-3 h-[2px] w-full bg-gray-900">
                                <div 
                                    className="h-full bg-purple-500 group-hover:shadow-[0_0_8px_#a855f7]" 
            ...
            {/* FILA 3: TENDENCIAS Y DEMANDA */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Tendencias de Búsqueda */}
                <section className="bg-[#111] p-6 rounded-lg border border-gray-800">
                    <h2 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] flex items-center gap-2 mb-6">
                        <TrendingUp size={14} className="text-emerald-500" /> Búsquedas Populares
                    </h2>
                        ...
                        {stats.top_searches.length > 0 ? stats.top_searches.map((item, idx) => (
                            <div key={idx} className="space-y-2 group">
                                ...
                                <div className="h-1 w-full bg-gray-900 rounded-full overflow-hidden">
                                    <div className="h-full bg-emerald-500 transition-all duration-1000" style={{ width: `${(item.count / maxSearch) * 100}%` }}></div>
                                </div>
                            </div>
                ...
                <section className="bg-[#111] p-6 rounded-lg border border-gray-800">
                    <h2 className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] flex items-center gap-2 mb-6">
                        <Package size={14} className="text-orange-500" /> Alertas de Reabastecimiento
                    </h2>
                    <div className="space-y-4">
                        {stats.stock_demands.length > 0 ? stats.stock_demands.map((item, idx) => (
                            <div key={idx} className="space-y-2 group">
                                <div className="flex justify-between text-[11px]">
                                    ...
                                    <span className="text-gray-500 font-mono">{item.total} solicitudes</span>
                                </div>
                                <div className="h-1 w-full bg-gray-900 rounded-full overflow-hidden">
                                    <div className="h-full bg-orange-500 transition-all duration-1000" style={{ width: `${(item.total / maxDemand) * 100}%` }}></div>
                                </div>
                            </div>
